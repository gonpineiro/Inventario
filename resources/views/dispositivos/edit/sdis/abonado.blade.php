          ...
          <form action="/update_abonado/{{$abonado->id}}" method="post" name="form">
            ...
            <div class="form-row">
              <div class="form-group col-md-2">
                ...
                <input type="text" class="form-control uppercase" name="numero" value="{{$abonado->numero}}" required>
              </div>
              <div class="form-group col-md-4">
                <label for="modelo_id">Departamento/Cliente </label> <a href="/departaments" target=_blank> +</a>
                <select class="form-control" name="departament_id">
                  <option value="{{$abonado->departament_id}}">{{$abonado->departament->name}}</option>
                  @foreach ($departaments as $departament)
                  <option value="{{$departament->id}}">{{$departament->name}} - C: {{$departament->cliente->name}}
                  </option>
                  @endforeach
                </select>
              </div>
            ...
            <div class="form-row">
              ...
              <div class="form-group col-md-2">
                ...
                <input type="text" min="10" class="form-control" value="{{$abonado->localidad}}" name="localidad"
                  required>
              </div>
              <div class="form-group col-md-2">
                <label for="localidad">Teléfono</label>
                <input type="number" min="10000" class="form-control" value="{{$abonado->telefono}}" name="telefono"
                  required>
              </div>
            </div>
            ...
            <div class="form-row">
              ...
              <div class="form-group col-md-3">
                ...
                <input type="palabra_clave" class="form-control" value="{{$abonado->palabra_clave}}"
                  name="palabra_clave" required>
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group col-md">
                ...
                <textarea rows="10" cols="50" type="text" class="form-control"
                  name="comentario">{{$abonado->comentario}} </textarea>
              </div>
            </div>
