            ...
            <div class="form-row">
              <div class="form-group col-md-5">
                ...
                <select class="form-control" name="cctv_id">
                  @if ($host->host->host_type_id == 40) <option value="{{$host->cctv_id}}">P: {{$host->host->id}} - AB:
                    {{$host->host->abonado->numero}} - C: {{$host->host->abonado->departament->cliente->name}}</option>
                  @endif
                  @if ($host->host->host_type_id == 41) <option value="{{$host->cctv_id}}">E: {{$host->host->id}} - P:
                    {{$host->host->host->id}} - AB: {{$host->host->host->abonado->numero}} - C:
                    {{$host->host->host->abonado->departament->cliente->name}}</option> @endif
                  @if ($host->host->host_type_id == 44) <option value="{{$host->cctv_id}}">T: {{$host->host->id}} - P:
                    {{$host->host->host->id}} - AB: {{$host->host->host->abonado->numero}} - C:
                    {{$host->host->host->abonado->departament->cliente->name}}</option> @endif

                  <option disabled>Paneles</option>
                  @foreach ($panel_alarms as $panel_alarm)
                  <option value={{$panel_alarm->id}}>P: {{$panel_alarm->id}} - AB: {{$panel_alarm->abonado->id}} - C:
                    {{$panel_alarm->abonado->departament->cliente->name}}</option>
                  @endforeach

                  <option disabled>Expansoras</option>
                  @foreach ($expansoras as $expansora)
                  <option value={{$expansora->id}}>E: {{$expansora->id}} - P: {{$expansora->host->id}} - AB:
                    {{$expansora->host->abonado->id}} - C: {{$expansora->host->abonado->departament->cliente->name}}
                  </option>
                  @endforeach

                  <option disabled>Teclados</option>
                  @foreach ($teclados as $teclado)
                  <option value={{$teclado->id}}>E: {{$teclado->id}} - P: {{$teclado->host->id}} - AB:
                    {{$teclado->host->abonado->id}} - C: {{$teclado->host->abonado->departament->cliente->name}}
                  </option>
                  @endforeach
