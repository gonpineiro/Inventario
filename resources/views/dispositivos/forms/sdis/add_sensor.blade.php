              ...
              <div class="form-group col-md-5">
                ...
                <select class="form-control" name="cctv_id" required>
                  ...
                  <option value={{$expansora->id}}>E: {{$expansora->id}} - P: {{$expansora->host->id}} - AB:
                    {{$expansora->host->abonado->numero}} - C: {{$expansora->host->abonado->departament->cliente->name}}
                  </option>
                  @endforeach
                  <option disabled>Tecladdos</option>
                  @foreach ($teclados as $teclado)
                  <option value={{$teclado->id}}>T: {{$teclado->id}} - P: {{$teclado->host->id}} - AB:
                    {{$teclado->host->abonado->numero}} - C: {{$teclado->host->abonado->departament->cliente->name}}
                  </option>
                  @endforeach
                </select>
              </div>
